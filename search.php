                        ...
                        <div style="margin-top: 50px;">
                            <form method="post" action="search.php">
                                ...
                                if ($_SESSION['admin'] == 1) {
                                    ...
                                    "; // works but only with existing genres in the database
                                }
                                ?>
                            </form>
                        </div>
                            ...
                            <li><span class='strip'></span>
                                <?php
                                    ...
                                    if (isset($_POST['search'])) {
                                        ...
                                        $searchquery = "SELECT games.id as gamesid, games.title as gamesTitle, genres.id as genresid, genres.title as genresTitle, games.image, games.rating FROM games INNER JOIN genres ON games.genre = genres.id WHERE games.title LIKE '%$search%' OR genres.title LIKE '%$search%' ORDER BY gamesid;";
                                        $result = mysqli_query($con, $searchquery);
                                        $searchQueryResult = mysqli_num_rows($result);
                                    } else {
                                        $searchquery = "SELECT games.id as gamesid, games.title as gamesTitle, genres.id as genresid, genres.title as genresTitle, games.image, games.rating FROM games INNER JOIN genres ON games.genre = genres.id ORDER BY gamesid;";
                                        $result = mysqli_query($con, $searchquery);
                                        $searchQueryResult = mysqli_num_rows($result);
                                    }
                                    if ($searchQueryResult > 0) {
                                    	while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            if (isset($_POST['fps']) && $_POST['fpshidden'] == $row['genresid']) {
                                            ...
                                            } elseif (isset($_POST['rpg']) && $_POST['rpghidden'] == $row['genresid']) {
                                                echo '<form method="get" action="gamePage.php">   
                                            <div class="description-of-games">
                                                Genre: '.$row["genresTitle"].'<br />Rating: '.$row["rating"].' 
                                            </div>
                                            ...
                                            } elseif(isset($_POST['sim']) && $_POST['simhidden'] == $row['genresid']) {
                                                echo '<form method="get" action="gamePage.php">   
                                            <div class="description-of-games">
                                                Genre: '.$row["genresTitle"].'<br />Rating: '.$row["rating"].' 
                                            </div>
                                            ...
                                            } elseif(isset($_POST['str']) && $_POST['strhidden'] == $row['genresid']) {
                                                echo '<form method="get" action="gamePage.php">   
                                            <div class="description-of-games">
                                                Genre: '.$row["genresTitle"].'<br />Rating: '.$row["rating"].' 
                                            </div>
                                            ...
                                            } elseif(isset($_POST['???']) && $_POST['???hidden'] == $row['genresid']) {
                                                echo '<form method="get" action="gamePage.php">   
                                            <div class="description-of-games">
                                                Genre: '.$row["genresTitle"].'<br />Rating: '.$row["rating"].' 
                                            </div>
                                    ...
                                    if (isset($_POST['admin-button'])) {
                                        $adminid = $_POST['admin-id-submit'];
                                        $adminName = $_POST['admin-name-submit'];
                                        $adminImage = $_POST['admin-image-submit'];
                                        $adminGenres = $_POST['admin-genre-submit-s'];
                                        $adminRating = $_POST['admin-rating-submit'];
                                        $adminSQL = "INSERT INTO games (id, title, image, genre, rating) VALUES (".$adminid.",'".$adminName."', '".$adminImage."', '".$adminGenres."', ".$adminRating.")";
